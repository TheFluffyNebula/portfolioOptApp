...
def GetTimeList(start_date, end_date, TimeDeltaHours=1):
    ...
    while current_date <= end_date:
        TimeList.append(current_date) #Adds the current date into the previously empty TimeList list above
        current_date += timedelta(hours=TimeDeltaHours) #TimeDeltaHours is the increment that will be used as we move beyond the current_date. Here TimeDeltaHours = 1
        
    return TimeList

def GetDepthData(InputDataPath):   

    Depth_NETCDF = xr.open_dataset(InputDataPath+"./Depths.nc") #xr library is used here since the data is in a .nc format
    
    Lat=Depth_NETCDF.lat.data #extracts lat data from the .nc file data as Numpy array using the .data function
    Long=Depth_NETCDF.lon.data
    RefDepth=np.reshape(Depth_NETCDF.elevation.data,-1) #The -1 here flattens the array into 1 dimension - a column of the depths with i number of rows
    RefDepth_LatLong = np.array(list(product(Lat, Long)))
    
    return RefDepth, RefDepth_LatLong    


def GetDepthCountours(DepthsIn=[30, 100, 500], LatMaxMin=(33.3, 37.2)):

    RefDepth, RefDepth_LatLong = GetDepthData("./InputData/")
    RefDepth=RefDepth*-1
    IdxIn=(RefDepth_LatLong[:,0]>LatMaxMin[0])* (RefDepth_LatLong[:,0]<LatMaxMin[1])*  (RefDepth>0)
    RefDepth_LatLong=RefDepth_LatLong[IdxIn]
    RefDepth=RefDepth[IdxIn]

    LatRange=np.arange(LatMaxMin[0]+0.05, LatMaxMin[1]-0.05, 0.05)

    #Get the latitude and longitude closest to the reference depth
    Depth_Curves=[]
    for TargetDepth in DepthsIn:
        RefDepth_LatLong_Closest=[]
        LatLong_Depth_Curve_i=[]

        Old_i_lat=-99
        Old_i_lon=-99
        for Lat in tqdm(LatRange):

            LatIdxs=np.where((np.abs(Lat-RefDepth_LatLong[:,0]))<=0.01)[0]
            if sum(LatIdxs)>=2:

                Tmp_Lat=RefDepth_LatLong[LatIdxs,0]
                Tmp_Lon=RefDepth_LatLong[LatIdxs,1]
                Tmp_Depth=RefDepth[LatIdxs]
                
                if np.min(np.abs(Tmp_Depth-TargetDepth))<TargetDepth*0.1:
                    Idx=np.argmin(np.abs(Tmp_Depth-TargetDepth))
                    i_lat=Tmp_Lat[Idx]
                    i_lon=Tmp_Lon[Idx]
                    if Old_i_lat<i_lat:
                        LatLong_Depth_Curve_i.append([i_lat, i_lon])
                        Old_i_lat=i_lat
            

        Depth_Curves.append(np.array(LatLong_Depth_Curve_i))

    return Depth_Curves, DepthsIn
...
def PlotGeneralGeoData(LatLong, Y_Variable, GeoDataPath="./InputData/CoastLine/", ColorBarTitle=None, Title=None, SavePath=None, s=6, LatMaxMin=(33.3, 37.2), LongMaxMin=(-78.7, -74.3)):
    ...
    df = gpd.read_file(ShapeFileCoast, engine="pyogrio")
    df1 = gpd.read_file(ShapeFileStates, engine="pyogrio")
...
def ChangeTimeSpaceResolution(ReferenceDataPath, CurrentTimeResolution, NewTimeResolution, StepsPerDegree, StartDateTime, EndDateTime, NewSavePath=None):
    ...
    if StepsPerDegree==-1:#Dont  change spatial resolution
        ResolutionKm=Data["ResolutionKm"]
        ResolutionDegrees=Data["ResolutionDegrees"]
        NumberOfCellsPerSite=Data["NumberOfCellsPerSite"]
        if NewSavePath!=None:
            np.savez(NewSavePath, Energy_pu=NewEnergy_Pu_time, RawResource=NewRawResource_time, TimeList=NewTimeList, LatLong=LatLong, Depth=Depth,
                DistanceShore=DistanceShore, CAPEX_site=CAPEX_site, OPEX_site=OPEX_site, AnnualizedCost=AnnualizedCost, NumberOfCellsPerSite=NumberOfCellsPerSite,RatedPower=RatedPower, 
                ResolutionDegrees=ResolutionDegrees, ResolutionKm=ResolutionKm)

        return NewEnergy_Pu_time, NewTimeList, NewRawResource_time, LatLong, Depth, DistanceShore, CAPEX_site, OPEX_site, AnnualizedCost, NumberOfCellsPerSite, RatedPower        
        
        
    if StepsPerDegree!=-1:#Change spatial resolution
...
def PlotEfficientFrontier(SolutionPaths, Legend, Title, linestyle=None,ColorList=None,Marker=None, SavePath=None, Hull=True,  bbox_to_anchor=(1.05, 1)):
    ...
    if linestyle==None:
        ...
        Marker=[None,None,None,None, None,None,None,None, None,None,None,None, None,None,None,None]
    ...
    for i, SolutionPath in enumerate(SolutionPaths):
        ...
        if Hull==True:
            hull = ConvexHull(np.column_stack((MWAvg,LCOE)))    
            points=np.column_stack((MWAvg,LCOE))

            plt.plot(points[hull.vertices,0][1:],points[hull.vertices,1][1:], label=Legend[i],linestyle=linestyle[i],color=ColorList[i],marker=Marker[i],markerfacecolor='none')
            
        else:
            plt.plot(MWAvg,LCOE, label=Legend[i],linestyle=linestyle[i],color=ColorList[i],marker=Marker[i],markerfacecolor='none')


    plt.xlabel("MW Avg - Delivered to Shore")
    plt.ylabel("LCOE [$/MWh]")
    plt.legend(loc='upper left', bbox_to_anchor=bbox_to_anchor, borderaxespad=0., frameon=False)
...
def PlotTurbineLocations(SolutionPath, Legend, PathDataUnderLayer, LegendUnderLayerColorbar, StateCountoursPath, UnderLayerVariable=["Energy_pu", "RawResource","RawResource"],
    ...
    ColorList=["black","green","blue","orange","red","purple"]

    LegendIn=[]
    for i, path in enumerate(SolutionPath):
        data=np.load(path,allow_pickle=True)
        SolutionLCOE = data["Save_LCOE_Achieved"].astype(float)  # Remove [:-1] slice
        if len(SolutionLCOE) == 0:
            raise ValueError("No LCOE values found in portfolio file")
        SolutionIDX = 0
        ...
        if len(data["PathWindDesigns"])>0:
            ...
            for PathWindDesigns in data["PathWindDesigns"]:
                    #Remove very low LCOEs for kites
            ...
            IdxInWind=np.where(LatLongWind[:,0]>=34)[0]
            LatLongWind=LatLongWind[IdxInWind,:]
            ...
            for j in range(len(IDX_Designs)-1):
                ...
                plt.scatter(LatLong_tmp[:,1], LatLong_tmp[:,0], marker=(3, 0, RangeOfRotations[i+j]), s=300, edgecolors=ColorList[i+j], facecolors='none',alpha=1, label=Legend[i][0][j])
        ...
        if len(data["PathWaveDesigns"])>0:
            ...
            for j in range(len(IDX_Designs)-1):
                ...
                #if len(LatLong_tmp)>0:
                #plt.scatter(LatLong_tmp[:,1], LatLong_tmp[:,0], marker=(4, 0, RangeOfRotations[i+j]), s=300, edgecolors=ColorList[i+j], facecolors='none',alpha=1, label=Legend[i][1][j]) 
                plt.scatter(LatLong_tmp[:,1], LatLong_tmp[:,0], marker=RangeOfRotations[j+i], c=ColorList[i+j], s=300,  alpha=1, label=Legend[i][1][j]) 
        ...
        if len(data["PathKiteDesigns"]) > 0:
            ...
            for PathKiteDesigns in data["PathKiteDesigns"]:
                ...
                AnnualizedCostKite = KiteData["AnnualizedCost"]
                RatedPowerKiteTurbine = np.full(len(KiteData["NumberOfCellsPerSite"]), float(KiteData["RatedPower"]))
                LCOE_Kite = AnnualizedCostKite*1e6/((KiteData['Energy_pu'].mean(axis=0)*RatedPowerKiteTurbine).mean()*8760)
                IdxInKites = np.where(LCOE_Kite < 100)[0]
                
                LatLongKite = np.concatenate((LatLongKite, KiteData["LatLong"][IdxInKites,:]))
                IDX_Designs.append(IDX_Designs[-1] + len(IdxInKites))

            for j in range(len(IDX_Designs)-1):
                if IDX_Designs[1+j] > len(Y_Kite):
                    continue
                y_kite_segment = Y_Kite[IDX_Designs[0+j]:IDX_Designs[1+j]]
                LatLong_tmp = LatLongKite[IDX_Designs[0+j]:IDX_Designs[1+j],:][y_kite_segment > 0,:]
                plt.scatter(LatLong_tmp[:,1], LatLong_tmp[:,0], marker=RangeOfRotations[j+i], 
                        c=ColorList[i+j], s=300, alpha=1, label=Legend[i][2][j])
